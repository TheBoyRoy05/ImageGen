...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(levelname)s: %(message)s", datefmt="%I:%M:%S"
)
...
class Diffusion:
    def __init__(
        self, noise_steps=1000, beta_start=1e-4, beta_end=0.02, img_size=64, device="cuda"
    ):
    ...
    def sample(self, model, n):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(
                reversed(range(1, self.noise_steps)), position=0, desc="DDPM Sampling..."
            ):
                ...
                x = (
                    1 / torch.sqrt(alpha) * (x - beta / torch.sqrt(1 - alpha_hat) * predicted_noise)
